...
//	private Invitation inv;
	...
	public GameObject dialogUser;
	public GameObject dialogProfileImage;
...
//		HeyzapAds.ShowMediationTestSuite();
	...
	void DisplayUserName(IResult result){
		if (dialogUser != null) {
			Text UserName = dialogUser.GetComponent<Text> ();
			if (result.Error == null) {
				GameData.Instance.fbUserName = result.ResultDictionary ["first_name"].ToString();
				UserName.text = "Hello, " + GameData.Instance.fbUserName;
			} else {
	...
	void DisplayUserProfile(IGraphResult result){
		...
		if (result.Texture != null) {
			Image ProfilePic = dialogProfileImage.GetComponent<Image> ();

			GameData.Instance.fbProfileImage = Sprite.Create (result.Texture, new Rect (0, 0, 128, 128), new Vector2 ());
			ProfilePic.sprite = GameData.Instance.fbProfileImage;
